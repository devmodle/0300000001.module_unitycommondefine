...
/** 에디터 접근자 */
[InitializeOnLoad]
public static partial class CAccessEditor
{
	#region 클래스 변수
	...
	#region 클래스 프로퍼티
	public static bool IsAppleMSeries => SystemInfo.processorType.ToUpper().Contains(KCDefineEditor.B_TOKEN_APPLE_M_SERIES);
	public static bool IsEnableUpdateState => !BuildPipeline.isBuildingPlayer && !EditorApplication.isUpdating && !EditorApplication.isCompiling && !EditorApplication.isPlayingOrWillChangePlaymode && !CAccessEditor.m_bIsImportAssets;
	#endregion // 클래스 프로퍼티

	#region 클래스 함수
	/** 생성자 */
	static CAccessEditor()
	{
		AssetDatabase.importPackageStarted -= CAccessEditor.OnStartAssetImport;
		AssetDatabase.importPackageStarted += CAccessEditor.OnStartAssetImport;

		AssetDatabase.importPackageCompleted -= CAccessEditor.OnCompleteAssetImport;
		AssetDatabase.importPackageCompleted += CAccessEditor.OnCompleteAssetImport;

		AssetDatabase.importPackageCancelled -= CAccessEditor.OnCancelAssetImport;
		AssetDatabase.importPackageCancelled += CAccessEditor.OnCancelAssetImport;

		AssetDatabase.importPackageFailed -= CAccessEditor.OnFailAssetImport;
		AssetDatabase.importPackageFailed += CAccessEditor.OnFailAssetImport;
	}
	...
	{
		string oDirName = Path.GetDirectoryName(KCDefineEditor.B_DIR_P_RES_PREFABS).Replace(KCDefine.B_TOKEN_R_SLASH, KCDefine.B_TOKEN_SLASH);
		string oPrefabPath = string.Format(KCDefine.B_TEXT_FMT_3_SLASH_COMBINE, oDirName, CAccessEditor.GetSceneDirName(a_stScene), string.Format(KCDefine.B_TEXT_FMT_2_UNDER_SCORE_COMBINE, a_stScene.name, a_oName));
	...
	{
		return KCDefineEditor.B_DIR_N_SCENE_DICT.ExGetVal(a_stScene.name, (!a_stScene.name.Contains(KCDefineEditor.B_EDITOR_SCENE_N_PATTERN_A) && !a_stScene.name.Contains(KCDefineEditor.B_EDITOR_SCENE_N_PATTERN_B)) ? Path.GetDirectoryName(KCDefineEditor.B_DIR_P_SUB_UNITY_PROJ).Replace(KCDefine.B_TOKEN_R_SLASH, KCDefine.B_TOKEN_SLASH) : Path.GetDirectoryName(KCDefineEditor.B_DIR_P_SUB_UNITY_PROJ_EDITOR).Replace(KCDefine.B_TOKEN_R_SLASH, KCDefine.B_TOKEN_SLASH));
	}

	/** iOS 프로젝트 이름을 반환한다 */
	public static string GetiOSProjName(EiOSType a_eType)
	{
		return KCDefineEditor.B_IOS_APPLE_BUILD_PROJ_N_JENKINS;
	}

	/** iOS 빌드 결과 경로를 반환한다 */
	public static string GetiOSBuildOutputPath(EiOSType a_eType, string a_oBuildFileExtension)
	{
		return string.Format(KCDefineEditor.B_BUILD_OUTPUT_P_FMT_IOS, CAccess.GetPlatformiOS(a_eType), a_oBuildFileExtension);
	}
	...
	public static string GetAndroidProjName(EAndroidType a_eType)
	{
		switch(a_eType)
		{
			case EAndroidType.AMAZON:
				return KCDefineEditor.B_ANDROID_AMAZON_BUILD_PROJ_N_JENKINS;
		}

		return KCDefineEditor.B_ANDROID_GOOGLE_BUILD_PROJ_N_JENKINS;
	}

	/** 안드로이드 빌드 결과 경로를 반환한다 */
	public static string GetAndroidBuildOutputPath(EAndroidType a_eType, string a_oBuildFileExtension)
	{
		return string.Format(KCDefineEditor.B_BUILD_OUTPUT_P_FMT_ANDROID, CAccess.GetPlatformAndroid(a_eType), a_oBuildFileExtension);
	}
	...
	public static string GetStandaloneProjName(EStandaloneType a_eType)
	{
		switch(a_eType)
		{
			...
			{
				return KCDefineEditor.B_STANDALONE_WNDS_STEAM_BUILD_PROJ_N_JENKINS;
			}
		}

		return KCDefineEditor.B_STANDALONE_MAC_STEAM_BUILD_PROJ_N_JENKINS;
	}
	...
	{
		return string.Format(KCDefineEditor.B_BUILD_OUTPUT_P_FMT_STANDALONE, CAccess.GetPlatformStandalone(a_eType), a_oBuildFileExtension);
	}
	...
	{
		CAccessEditor.m_bIsImportAssets = true;
	}

	/** 에셋 임포트가 완료되었을 경우 */
	private static void OnCompleteAssetImport(string a_oAssetName)
	{
		CAccessEditor.m_bIsImportAssets = false;
	}

	/** 에셋 임포트가 취소되었을 경우 */
	private static void OnCancelAssetImport(string a_oAssetName)
	{
		CAccessEditor.OnCompleteAssetImport(a_oAssetName);
	}
	...
	{
		var oAssets = CAccessEditor.FindAssets<T>(a_oFilter, a_oSearchPathList);
