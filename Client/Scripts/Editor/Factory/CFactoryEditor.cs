...
/** 에디터 팩토리 */
public static class CFactoryEditor
{
	#region 클래스 함수
	/** 디렉토리를 생성한다 */
	public static void MakeDirectories(string a_oDirPath, bool a_bIsAssert = true)
	{
		CFunc.Assert(!a_bIsAssert || (a_oDirPath.ExIsValid() && a_oDirPath.Contains(KCDefineEditor.B_DIR_P_ASSETS)));

		// 경로가 유효 할 경우
		if(a_oDirPath.ExIsValid() && a_oDirPath.Contains(KCDefineEditor.B_DIR_P_ASSETS))
		{
	...
	{
		CFunc.Assert(a_oAssetPath.ExIsValid() && CAccessEditor.IsExistsAsset(a_oAssetPath));
	...
	public static GameObject CreatePrefabInstance(string a_oName, string a_oObjPath, GameObject a_oParent)
	{
		...
		return CFactoryEditor.CreatePrefabInstance(a_oName, Resources.Load<GameObject>(a_oObjPath), a_oParent);
	}
	...
	#region 제네릭 클래스 함수
	/** 에디터 윈도우를 생성한다 */
	public static T CreateWndEditor<T>(string a_oName) where T : EditorWindow
	{
